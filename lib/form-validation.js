	...
	return function(req, res, next) {
		...
		if (staticContentRegExp.test(resource)) { // Don't check static content
			...
			if (validationErrors.length > 1) {
				...
				logger.debug("FormaValidation module# Errores de validacion establecidos en las variables locales de la respuesta: " + sys.inspect(validationErrors));
			}
			res.local('validationErrors', validationErrors);
			return next();
		}
	}
};
